...
function SignUp() {
    ...
    const handleSignup = async (e) => {
        ...
        if (!result.success) {
            ...
            const errors = [
                formatted.firstName?._errors[0],
                formatted.lastName?._errors[0],
        ...
        try {
            setLoading(true)
            const response = await fetch(`${API_URL}/api/v1/${role}/signup`, {
                method: "POST",
    ...
    return (
        <div className={`min-h-screen ${pageBackground} flex items-center justify-center px-4 py-8`}>
            ...
            <div className={`w-full max-w-md bg-slate-900/80 backdrop-blur-xl border ${accentBorder} rounded-2xl md:rounded-3xl p-5 md:p-7 ${cardGlow}`}>
                ...
                <h1 className={`text-3xl font-bold text-center ${accentText}`}>Create Account</h1>
                ...
                <p className="text-slate-400 text-center mt-6 text-md">
                    ...
                    <Link to="/signin" className={`${accentText} hover:underline`}>Sign In</Link>
                </p>
